...
local function visit_node(root, level, lang_tree, headings, opt, stats)
	...
	elseif node_name == "line" then
		...
		local div = opt.old
			and root:child(0)
			and vim.tbl_contains({ "column_heading", "h1", "h2", "h3" }, root:child(0):type())
...
local function get_helpfiles(include, help_dir)
	local dir = ""
	if string.len(help_dir) == 0 then
		dir = dir .. "./build/runtime/doc"
	else
		dir = dir .. help_dir
	end
	local rv = {}
	for f, type in vim.fs.dir(dir) do
		if vim.endswith(f, ".txt") and type == "file" and (not include or vim.tbl_contains(include, f)) then
...
function M.gen(help_dir, to_dir, include, commit, parser_path)
	...
	helpfiles = get_helpfiles(include, help_dir)
